...
class ElexonClient:
    """Client for fetching data from the Elexon Insights API.

    This class stores a `requests.Session` for connection reuse and exposes the
    same download and date-chunking behaviour as the previous module-level
    function.

    Attributes:
        base_url: Base API URL used to build requests.
        session: HTTP session used to perform requests (reused across calls).
        datasets: pandas DataFrame with the registry metadata (same as
                  `elexonapi.datasets.datasets`).
    """
    ...
    # Convenience accessors to keep the same discoverability surface
    @property
    def datasets(self) -> pd.DataFrame:
        """Return the dataset registry DataFrame."""
        return self._datasets

    def browse(self) -> Any:
        """Show an interactive view of the datasets (Jupyter-friendly)."""
        return browse_fn(datasets=self._datasets)

    def help(self, alias: str) -> dict:
        """Print and return metadata for a dataset alias."""
        return help_fn(alias, datasets=self._datasets)

    def _resolve_operation(self, alias: str) -> str:
        """Resolve alias using the client's dataset registry."""
    ...
    def download(
        ...
        **params: Any,
        ...
        """Download data for the provided dataset alias.

        Behaviour mirrors the previous functional API but uses the client's
        session and registry. The method will raise `ValueError` for missing or
        unknown parameters and will chunk long date ranges automatically.
        """
        ...
        # Support `_from` keyword to avoid Python reserved word `from`
        if "_from" in params:
...
# Helper functions (unchanged behaviour but typed for clarity)
